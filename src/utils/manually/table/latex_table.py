...
def generate_latex_table(path: str, metrics: list = ["goal", "nmi"]):
    """
    ...
    # Do the same for "NMI"
    df.loc[(df["Metric"] == "NMI").idxmax(), "Metric"] = (
        r"\multirow{3}{*}{\textbf{NMI}}"
    )
    ...
    latex_str = latex_str.replace(
        r"\multirow{3}{*}{\textbf{NMI}}", r"\hline \hline \multirow{3}{*}{\textbf{NMI}}"
    )
    print(latex_str)
...
if __name__ == "__main__":
    ...
    ALG = algs[0]
    TAU = taus[0]
    PATH = f"test/{DATASET}/{ALG}/node_hiding/tau_{TAU}/allBetas_evaluation_node_hiding_mean_std.json"
